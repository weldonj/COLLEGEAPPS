...
class Student(object):
    ...
    def precision_mark(self):
        adjusted_marks = [self.__mods[code].ects * self.__marks[code] for code in self.__mods]
        return sum(adjusted_marks) / sum([self.__mods[code].ects for code in self.__mods])

    def passed(self):
        return all([self.__marks[code] >= 40 for code in self.__mods])

    def passed_by_compensation(self):
        credits = sum([self.__mods[code].ects for code in self.__mods])
        earned_credits = sum([self.__mods[code].ects for code in self.__mods if self.__marks[code] >= 40])
        failed_credits = credits - earned_credits
